...
def get_the_number_of_Nans(df):
    return df.isnull().sum()

print("NUMBER OF NANS:")
print(get_the_number_of_Nans(df))

def fill_Nans_with_random_values_and_create_new_dataset():
    new_dataset = df.copy()
    new_dataset.fillna(rand.randint(0,100), inplace = True)
    return new_dataset

print("\n new_dataset \n")
new_dataset = fill_Nans_with_random_values_and_create_new_dataset()

def get_the_steps_per_day(df):
    steps_per_day = df.groupby("date").sum()["steps"]
    return steps_per_day.to_frame()

print("steps_per_day")
print(get_the_steps_per_day(df))

def histogram_of_the_total_steps_per_day(df):
    steps_per_day = get_the_steps_per_day(df)
...
histogram_of_the_total_steps_per_day(df)

def get_mean_of_the_steps_per_day(df):
    mean_per_day=df.groupby("date").mean()["steps"]
    return mean_per_day.to_frame().dropna()

print("MEAN")
print(get_mean_of_the_steps_per_day(df))
def get_median_of_the_total_steps_per_day(df):
    mean_per_day=df.groupby("date").median()["steps"]
    return mean_per_day.to_frame().dropna()

print("MEDIAN")
print(get_median_of_the_total_steps_per_day(df))

#time series plot of the 5-minute interval (x-axis) and the average number of steps taken, averaged across all days (y-axis)
def plot_mean_of_total_steps_per_day():
    plt.plot(get_mean_of_the_steps_per_day(df), "r")
...
histogram_of_the_total_steps_per_day(new_dataset)
print("MEAN OF NEW DATASET")
print(get_mean_of_the_steps_per_day(new_dataset))
print("MEDIAN OF NEW DATASET")
print(get_median_of_the_total_steps_per_day(new_dataset))
